...
- (NSString *)stringByPigLatinization {
    ...
    for (int i = 0; i < [words count]; ++i) {
        ...
        words[i] = ([words[i] length] >= 2 && [clusters containsObject:[[words[i] substringToIndex:2] lowercaseString]]) ? [[[words[i] substringFromIndex:2] stringByAppendingFormat:@"%@ay", [words[i] substringToIndex:2]] capitalizedString] : ([vowels characterIsMember:[words[i] characterAtIndex:0]] || [tokens count] == 1) ? [[words[i] stringByAppendingString:@"ay"] capitalizedString] : [[[words[i] substringFromIndex:[tokens[0] length]] stringByAppendingFormat:@"%@ay", tokens[0]] capitalizedString];
    }
